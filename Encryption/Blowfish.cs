...
{
    /// <summary>
    /// Class providing blowfish decryption with modifications specific to TellTale implementation.
    /// Until TellTale archive version 7, standard Blowfish was used. Starting from version 7, a few
    /// modifications to the algorithm were implemented.
    /// </summary>
    public class Blowfish
    {
        ...
        private readonly bool modifiedBlowfish;
        ...
        public void Decipher(byte[] data, uint length)
        {
            Decipher(data, data, length);
        }
        ...
        /// <param name="dest">The array to write decrypted data to.</param>
        /// <param name="length">The amount to decrypt.</param>
        public void Decipher(byte[] source, byte[] dest, uint length)
        {
            if ((length % 8) != 0)
            ...
            for (int i = 0; i < length; i += 8)
            {
                ...
                // Opposite endianess (original):
                //xl = (uint)((data[i] << 24) | (data[i + 1] << 16) | (data[i + 2] << 8) | data[i + 3]);
                //xr = (uint)((data[i + 4] << 24) | (data[i + 5] << 16) | (data[i + 6] << 8) | data[i + 7]);
                var xl = (uint)((source[i]) | (source[i + 1] << 8) | (source[i + 2] << 16) | source[i + 3] << 24);
                var xr = (uint)((source[i + 4]) | (source[i + 5] << 8) | (source[i + 6] << 16) | source[i + 7] << 24);
                if (modifiedBlowfish)
                {
                    Decipher7(ref xl, ref xr);
                }
                else
                {
        ...
        /// <summary>
        /// Decrypts 8 bytes of data (1 block) using modified TellTale (version 7)
        /// </summary>
        /// <param name="xl">The left part of the 8 bytes.</param>
        /// <param name="xr">The right part of the 8 bytes.</param>
        private void Decipher7(ref uint xl, ref uint xr)
        {
            uint temp;
            short i;
