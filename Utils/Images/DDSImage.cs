...
namespace SCUMMRevLib.Utils
{
    public class DDSImage
    {
        ...
        public ImageByteArray32[] images;

        public int MipMapCount { get; private set; }
        public int Width { get { return header.dwWidth; } }
        public int Height { get { return header.dwHeight; } }

        private long streamDataPosition;
        ...
        public DDSImage(Stream stream)
        {
            using (BinaryReader reader = new BinaryReader(stream))
            {
                ...
                images = new ImageByteArray32[MipMapCount];

                streamDataPosition = stream.Position;
            }
        }

        public void Decode(Stream stream)
        {
            stream.Position = streamDataPosition;
            using (BinaryReader reader = new BinaryReader(stream))
            {
                bdata = reader.ReadBytes(header.dwPitchOrLinearSize);
        ...
        private ImageByteArray32 ReadBlockImage(byte[] data, int w, int h)
        {
        ...
        #region DXT1
        private ImageByteArray32 UncompressDXT1(byte[] data, int w, int h)
        {
